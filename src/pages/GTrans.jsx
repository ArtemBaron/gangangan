...
import { 
  ...
  TrendingUp, CheckCircle, CreditCard, RefreshCw, Users,
  FileCheck, Clock, Eye
} from 'lucide-react';
          ...
          <div className="grid lg:grid-cols-2 gap-16 items-center">
            ...
            >
              <h2 className="text-3xl md:text-4xl font-bold text-[#1e3a5f] mb-6">
                {language === 'en' ? 'Who is GTrans?' : 'Siapa GTrans?'}
              </h2>
              <p className="text-lg text-slate-600 mb-6">
                {language === 'en'
                  ? "GTrans is GAN's cross-border funds transfer (remittance) service for corporate clients. We enable multi-currency payouts and collections between entities in different countries."
                  : "GTrans adalah layanan transfer dana lintas batas (remitansi) GAN untuk klien korporat. Kami memungkinkan pembayaran dan koleksi multi-mata uang antar entitas di berbagai negara."
                }
              </p>
              <p className="text-lg text-slate-600">
                {language === 'en'
                  ...
                  : 'Beroperasi di bawah kerangka lisensi PSP Bank Indonesia untuk remitansi, memenuhi semua persyaratan kecukupan modal, manajemen risiko, kepemilikan, dan kontrol teknologi.'
                }
              </p>
            </motion.div>
            <motion.div
            ...
            {products.map((product, idx) => (
              ...
              >
                <Card className="h-full border-slate-200 hover:shadow-xl transition-all bg-white">
                  <CardContent className="p-8">
                    <div className="w-14 h-14 rounded-xl bg-[#f5a623]/20 flex items-center justify-center mb-6">
                      <product.icon className="w-7 h-7 text-[#f5a623]" />
                    </div>
                    <h3 className="text-xl font-bold text-[#1e3a5f] mb-4">{product.title}</h3>
